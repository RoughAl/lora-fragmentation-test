...
public:
    RadioEvent(Callback<void(uint8_t, void*, size_t)> aRxCallback) : rxCallback(aRxCallback) {}
    ...
    virtual void MacEvent(LoRaMacEventFlags* flags, LoRaMacEventInfo* info) {
        ...
        if (flags->Bits.Rx) {
            ...
            if (rxCallback) {
                rxCallback(info->RxPort, info->RxBuffer, info->RxBufferSize);
            }
        }
    }

private:
    Callback<void(uint8_t, void*, size_t)> rxCallback;
};
