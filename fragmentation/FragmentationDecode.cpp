...
void StoreRowInFlash( uint8_t* rowData, int index, int bank )
{
    ...
    //if ( index < sFotaParameter.M+1)
    // {
    // StoreLineImageInFlash(sFotaParameter.DataSize*index,rowData,sFotaParameter.DataSize);
    // }

    //EepromMcuWriteBuffer( sFotaParameter.DataSize*index, rowData, sFotaParameter.DataSize, bank );
}
/*!
...
{
    int i;
    for (i = 0; i < size-EXTRABYTELORAWAN ; i ++) {
        rowData[i]=rowData[i+EXTRABYTELORAWAN];
    }
}
...
void FotaParameterInit( int M, int Redundancy, int DataSize )
{
    ...
    for (i = 0; i < NBOFUTILFRAMEMAX; i++) {
        for ( j = 0; j < SIZEOFFRAMETRANSMITMAX ; j++) {
            FlashData[i][j]=0xFF;
        }
    }
}
...
{
    //EepromMcuReadBuffer( l*sFotaParameter.DataSize,rowData, sFotaParameter.DataSize, bank );
...
int FragmentationDecodeCore( uint8_t* rowData, int bank )
{
    ...
    printf("Receive Frame With Fcnt = %d\n",frameCounter);

    printf("RECEIVED\n");
    printf("%d/%d\n", frameCounter,sFotaParameter.M);
    printf("%d/%d kB\n", frameCounter*sFotaParameter.DataSize/1000,sFotaParameter.M*sFotaParameter.DataSize/1000);
    if ( frameCounter < sFotaParameter.LastReceiveFrameCnt) {
        return 0xFFFFFFFF;  // drop frame out of order
    }
    RemoveLoraExtraByte(rowData,sFotaParameter.DataSize + EXTRABYTELORAWAN  );// 2 for fnct
        ...
        if (first > 0) { //manage a new line in MatrixM2
            ...
            if ( m2l == sFotaParameter.NumberOfLoosingFrame ) { // then last step diagonalized
                if ( sFotaParameter.NumberOfLoosingFrame > 1 ) {
                    ...
                    for (int q=0; q<sFotaParameter.M; q++) {
                        for (int qq=0; qq<sFotaParameter.DataSize; qq++) {
                            FlashDataa[cptalc]=FlashData[q][qq];
